@extends("template.layouts.master",['page_title'=>"Edit User"])
@section("vendorCss")
    <!-- BEGIN: Vendor CSS-->
                ...
                <div class="card-body">
                    <div class="tab-content">
                        ...
                        <div class="tab-pane active" id="account" aria-labelledby="account-tab" role="tabpanel">
                            ...
                            <form class="" method="post" action="{{ route("users.update") }}" id="user_edit_form" enctype="multipart/form-data">
                                @csrf
                                @method("PUT")
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="basic-icon-default-email" class="form-control dt-email" required placeholder="john.doe@example.com" aria-label="john.doe@example.com" aria-describedby="basic-icon-default-email2" value="{{old('email')?old("email"):$user->email}}" name="email" />
                                            @error('email')
                                            <span class="error">{{ucwords($message)}}</span>
                                            @enderror
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="number" max="100" min="1" id="basic-icon-default-commission" class="form-control dt-uname" placeholder="25%" value="{{old('commission')?old("commission"):$user->commission}}" name="commission" />
                                            @error('commission')
                                            <span class="error">{{ucwords($message)}}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select id="user-role" required name="role" onchange="showTag(this.value)" class="form-control">
                                                ...
                                                @foreach(getRoles() as  $role)
                                                    <option {{ ((isset($user->roles->first()->id)) && $user->roles->first()->id == $role->id)?"selected":"" }} value="{{$role->id}}">{{ucwords($role->name)}}</option>
                                                @endforeach
                                            </select>
                                            @error('role')
                                            <span class="error">{{ucwords($message)}}</span>
                                    ...
                                    <div class="col-md-4 d-none" id="tagIdSection">
                                        <div class="form-group">
                                            <label class="form-label" for="tags">Tag ID
                                                @error('tags')
                                                <span class="error">{{ucwords($message)}}</span>
                                                @enderror
                                            </label>
                                            <select name="tags[]" class="text-light" multiple id="tags">
                                                <option disabled>Add Tags</option>
                                                @foreach(json_decode($user->tag_id)??array() as $tag)
                                                    <option selected value="{{ $tag }}">{{ $tag }}</option>
                                                @endforeach
                                            </select>
{{--                                            <input type="text" multiple id="basic-icon-default-tag_id" class="form-control dt-uname" min="0" placeholder="{{rand(1000000000,9999999999)}}" aria-label="{{rand(1000000000,9999999999)}}" aria-describedby="basic-icon-default-uname2" value="{{old('tag_id')?old("tag_id"):$user->tag_id}}" name="tag_id" />--}}
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label>Shops</label>
                                            <select id="shops" class="select2 form-control" name="shops[]" multiple>
                                                <optgroup label="Shops">
                                                    @if(Auth::user()->hasRole("Super Admin"))
                                                        @foreach(getShops() as $shop)
                                                            <option {{in_array($shop->id,$shops)?'selected':''}} value="{{$shop->id}}">{{ucwords(str_ireplace('-',' ',$shop->name))}}</option>
                                                        @endforeach
                                                    @else
