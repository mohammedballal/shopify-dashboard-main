@extends("template.layouts.master",['page_title'=>"Price Table List"])
@section("vendorCss")
    <!-- BEGIN: Vendor CSS-->
        ...
        <section class="app-shop-list">
            ...
            <div class="card p-2">
                ...
                <!-- Modal to add new shop starts-->
                    @include('template.layouts.views.addPriceTable')
...
{{--    <script src="{{ asset("app-assets/js/scripts/pages/app-user-list.js")}}"></script>--}}
    ...
    <script>
        function editShop(id) {
            let url = window.location.origin+'/price/'+id+'/edit';
            fetch(url)
                .then(response => response.json())
                .then(json =>{
                    console.log(json);
                    url = window.location.origin+'/price/update/'+json.id;
                    $('#shopTitle').text('Edit Price Table');
                    $('#sub_btn_shop_save_form').text('Update');
                    $('#addShop').attr('action',url);
                    $('#product_name').val(json.product_name);
                    $('#cost_usd').val(json.cost_usd);
                    $('#cost_brl').val(json.cost_brl);
                    $('#mark_up').val(json.mark_up);
        ...
        $("#cost_brl").keyup(function (){
            var current_rate = $("#current_rate").val()
            var cost_brl = $(this).val()
            $("#cost_usd").val(round(cost_brl / current_rate))
        });
        $(function () {
            'use strict';
            ...
            if (dtshopTable.length) {
                dtshopTable.DataTable({
                    ajax: "{{ route("price.index") }}", // JSON file to add data
                    columns: [
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            render: function (data, type, full, meta) {
                                var destroy = "/price/destroy/"+full['id'];
                                var edit = full['id'];
                                return (
                                    '<div class="btn-group">' +
