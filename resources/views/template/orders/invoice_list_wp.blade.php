...
{{--            @include('template.layouts.views.addUser')--}}
    ...
    <form action="{{ route("order.invoices.print.wp") }}" method="post" target="_blank" id="printInvoices">
        @csrf
        ...
        $(function () {
            ...
            if (dtUserTable.length) {
                dtUserTable.DataTable({
                    ajax: "{{ route("orders.list.wp") }}", // JSON file to add data
                    columns: [
                    ...
                    columnDefs: [
                        ...
                        ,
                        {
                            ...
                            render: function (data, type, full, meta) {
                                ...
                                var destroy = "#";
                                var edit = "#";
                                var invoice = "{{ url('/orders/invoice/wp') }}/"+`${full['id']}`;
                                return (
                                    '<a href="' +
                                    invoice +
                    ...
                    initComplete: function () {
                        ...
                        this.api()
                            ...
                            .every(function () {
                                var column = this;
                                var select = $(
                                    '<select id="FilterTransaction" class="form-control text-capitalize mb-md-0 mb-2xx"><option value=""> Select Order Status </option></select>'
                                )
