@extends("template.layouts.master",['page_title'=>"Shops List"])
@section("vendorCss")
    <!-- BEGIN: Vendor CSS-->
        ...
        <section class="app-shop-list">
            ...
            <div class="card p-2">
                ...
                <!-- Modal to add new shop starts-->
                    @include('template.layouts.views.addShop')
...
{{--    <script src="{{ asset("app-assets/js/scripts/pages/app-user-list.js")}}"></script>--}}
    ...
    <script>
        function editShop(id) {
            let url = window.location.origin+'/shop/edit/'+id;
            fetch(url)
                .then(response => response.json())
                .then(json =>{
                    console.log(json);
                    url = window.location.origin+'/shop/update/'+json.id;
                    $('#shopTitle').text('Edit Shop');
                    $('#sub_btn_shop_save_form').text('Update');
                    $('#addShop').attr('action',url);
                    $('#shopName').val(json.name);
                    $('#apiKey').val(json.api_key);
                    $('#api_pass').val(json.api_pass);
                    $('#api_version').val(json.api_version);
        ...
        $(function () {
            ...
            if (dtshopTable.length) {
                dtshopTable.DataTable({
                    ajax: "{{ route("shop.index") }}", // JSON file to add data
                    columns: [
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            render: function (data, type, full, meta) {
                                var destroy = "{{ route("shop.destroy") }}"+"?shop_id="+full['id'];
                                var edit = full['id'];
                                return (
                                    '<div class="btn-group">' +
